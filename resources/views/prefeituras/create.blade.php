@extends('layouts.app')

@section('page-title', 'Novo Cliente')
@section('page-subtitle', 'Cadastre a prefeitura para começar a adicionar contratos.')

@section('content')
<div class="overflow-hidden bg-white shadow-sm rounded-xl fade-in">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
        <h3 class="text-lg font-bold text-gray-700">Dados da Prefeitura</h3>
    </div>

    <div class="p-6">
        <form action="{{ route('prefeituras.store') }}" method="POST">
        @csrf

            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div class="col-span-1 md:col-span-2">
                    <label for="nome" class="block text-sm font-medium text-gray-700">Nome da Prefeitura <span class="text-red-500">*</span></label>
                    <input type="text" name="nome" id="nome" value="{{ old('nome') }}" required
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#115e59] focus:border-[#115e59]"
                        placeholder="Ex: Prefeitura Municipal de Currais">
                </div>

                <div>
                    <label for="cnpj" class="block text-sm font-medium text-gray-700">CNPJ <span class="text-red-500">*</span></label>
                    <input type="text" name="cnpj" id="cnpj" value="{{ old('cnpj') }}" required
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#115e59] focus:border-[#115e59]"
                        placeholder="00.000.000/0000-00">
                </div>

                <div>
                    <label for="endereco" class="block text-sm font-medium text-gray-700">Endereço / Cidade</label>
                    <input type="text" name="endereco" id="endereco" value="{{ old('endereco') }}"
                        class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#115e59] focus:border-[#115e59]">
                </div>
            </div>

            <div class="flex justify-end mt-8 space-x-4 pt-4 border-t border-gray-100">
                <a href="{{ route('prefeituras.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Cancelar</a>
                <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-[#062F43] rounded-lg hover:bg-[#084d6e] shadow-md">
                    Salvar e Ir para Contratos
                </button>
            </div>
        </form>
    </div>
</div>

@push('scripts')
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const cnpjInput = document.getElementById('cnpj');
        if (cnpjInput && typeof IMask !== 'undefined') {
            IMask(cnpjInput, { mask: '00.000.000/0000-00' });
        }
    });
</script>
@endpush
@endsection